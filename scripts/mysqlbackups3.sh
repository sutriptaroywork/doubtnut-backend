...
MYSQLHOST=35.200.228.199
MYSQLROOT=root
MYSQLPASS=root
S3BUCKET=dn-db-backup
FILENAME=backup
DATABASE='classzoo1'
...
# dump all databases
${MYSQLDUMPPATH}mysqldump --quick --host=${MYSQLHOST} --user=${MYSQLROOT} --password=${MYSQLPASS} ${DATABASE} > ${TMP_PATH}${FILENAME}.sql
