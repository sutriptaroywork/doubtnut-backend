...
const analyticsMysql = new database(config.mysql_analytics);
...
function getTeachersData(sClass, language) {
  ...
  console.log("teacher\n",sql);
  return analyticsMysql.query(sql);
}
...
async function createRecommendedPlaylist(data, sClass, language) {
    if(data && data.rows && data.rows.length){
        ...
        for (let k = 0; k < data.rows.length; k++) {
            // console.log(data.rows[k].subject.toLowerCase());
            // const carsColor1 = cardColor[data.rows[k].subject.toLowerCase()] ? cardColor[data.rows[k].subject.toLowerCase()] : "#E5E5E5";
            // console.log("carsColor1", carsColor1);
            if(data.rows[k].resource_reference && !qidKeys[data.rows[k].resource_reference] && Object.keys(qidKeys).length < 5){
                const obj = {
                    type: 'widget_child_autoplay',
                    data: {
                        id: `${data.rows[k].resource_reference}`,
                        assortment_id: data.rows[k].assortment_id,
                        page: 'LIVECLASS_FREE',
                        title1: `${_.startCase(data.rows[k].subject.toLowerCase())}, Class ${data.rows[k].class}`,
                        title2: `${data.rows[k].display}`,
                        image_url: `${data.rows[k].expert_image}`,
                        image_bg_card: `${subjetBgImageCard[data.rows[k].subject.toLowerCase()] || subjetBgImageCard.default}`,
                        subject: null,
                        color: '#ccddff',
                        ...
                        payment_deeplink: `doubtnutapp://vip?assortment_id=${data.rows[k].assortment_id}`,
                        card_width: '1.25',
                        card_ratio: '16:9',
                        ...
                        deeplink: '',
                        bottom_layout: {
                            title: `Class ${data.rows[k].class}, ${data.rows[k].topic}`,
                            title_color: '#504e4e',
                            sub_title: `${data.rows[k].count} attended`,
                        ...
                        title3: `By ${_.startCase(data.rows[k].expert_name.toLowerCase())}`,
                        title3_text_size: '12',
                ...
                finalData.push(obj);
                qidKeys[data.rows[k].resource_reference] = true;
            }
        }
...
async function createPopularTopicPlaylist(data, sClass, language){
    if(data && data.rows && data.rows.length){
        console.log("chapter data length", data.rows.length);
        const finalData = [];
        const subjData = data.rows.reduce((prev, curr) => {
        ...
        for (let k = 0; k < subjList.length; k++) {
            if(subjData[subjList[k]] && subjData[subjList[k]].length && subjData[subjList[k]][0].chapter_assortment_id && subjData[subjList[k]][0].subject !== "announcement"){
                const obj = {
                    // "deeplink": `doubtnutapp://course_details?id=${subjData[subjList[k]][0].chapter_assortment_id}`,
                    ...
                    "image_bg_color_one": "", // "#4ca4e3",
                    "image_bg_color_two": "", // "#176dab",
