...
function connectMongo() {
  return new Promise((resolve, reject) => {
    MongoClient.connect(config.mongo.database_cron_url,{ useUnifiedTopology: true },async function (err, client) {
        ...
        let newDataJson = await getCombinedNewData(db,oldData)
			  let sqlDataJson = await getCombinedSqlDataJson(db,mysql,mysqlRead,newDataJson)
        ...
        for(let i=0;i<oldData.length;i++){
          ...
          if(oldData[i].s_n_id == Number(oldData[i].s_n_id)){
            ...
            if(oldData[i].s_n_id != null && isNaN(oldData[i].s_n_id ) == false && oldData[i].s_n_id>0){
                ...
                let oldPerDayCount = await getLastHoursDetails(db, oldData[i].s_n_id)
...
async function getCombinedNewData(db,oldData){
	return new Promise(async function(resolve, reject) {
    try {
    	 let newDataJson =[], newDataSample
    	 for(let i=0;i<oldData.length;i++){
         newDataSample ={}
         if(oldData[i].s_n_id == Number(oldData[i].s_n_id)){
           ...
           if(oldData[i].s_n_id != null && isNaN(oldData[i].s_n_id ) == false && oldData[i].s_n_id>0){
              oldData[i].s_n_id = String(oldData[i].s_n_id)
             let newPerDayCount = await getLastHoursDetails(db, oldData[i].s_n_id)
...
async function getLastHoursDetails(db, s_n_id) {
  return new Promise(async function(resolve, reject) {
    try {
...
async function getCombinedSqlDataJson(db,mysql,mysqlRead,newDataJson){
	return new Promise(async function(resolve, reject) {
    try {
    	let sqlDataJson=[], percentageDelivered
    	for(let i =0; i<newDataJson.length; i++){
          if(newDataJson[i].s_n_id == Number(newDataJson[i].s_n_id)){
           ...
           if(newDataJson[i].s_n_id != null && isNaN(newDataJson[i].s_n_id ) == false && newDataJson[i].s_n_id>0){
        	...
        	let sqlReceivedUpdate = await updateReceivedCount(newDataJson[i].s_n_id,newDataJson[i].count, mysql)
           }
         }
        }
...
function updateReceivedCount(s_n_id,count, mysql){
