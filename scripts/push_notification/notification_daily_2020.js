...
async function connectMongo() {
    ...
    MongoClient.connect(config.mongo.database_url,{useNewUrlParser: true},{useUnifiedTopology: true},async function (err, c) {
        ...
        console.log("oldData")
        console.log(oldData)
        let oldDataJson =[], oldDataSample, newDataJson =[], newDataSample,sqlDataJson=[],percentageDelivered, question_id, views_videos, views_not_zero, classTotal=[], viewsTotal =[],
          notZeroClass=[], viewsNotZero=[],vClass=[], vTot=[], nZclass =[], nViews =[], locale =[], localeTot=[],  nLocale =[], notZeroLocale =[]
        ...
        for(var i =0; i<oldDataJson.length; i++){
          ...
          classTotal=[], viewsTotal =[], notZeroClass=[], viewsNotZero=[],vClass=[], vTot=[], nZclass =[], nViews =[], locale =[], localeTot=[],  nLocale =[], notZeroLocale =[]
                     ...
                     // sqlDataSample.views_total  =views_videos[0].total_view
                      if(typeof views_videos !='undefined'  && views_videos != null && views_videos !=''){
                        ...
                        for (var key in classTotal) {
                          var viewClassSample ={}
                          viewClassSample.class  = classTotal[key]
                          vClass.push(viewClassSample)
                        }
                      
                        for (var key in viewsTotal){
                          var viewsTotalSample ={}
                          viewsTotalSample.views_total  = viewsTotal[key]
                          vTot.push(viewsTotalSample)
                        }

                        for(var key in locale){
                          var localeTotalSample={}
                          localeTotalSample.locale = locale[key]
                          localeTot.push(localeTotalSample)
                        }
                       
                        var localeTotArray=[]
                        for(let z=0;z<localeTot.length;z++){
                          localeTotArray.push(localeTot[z].locale)
                        }

                       sqlDataSample.locale  = localeTotArray
                      
                        var classArray=[]
                        for(let z=0; z<vClass.length; z++)
                        {
                          
                          classArray.push(vClass[z].class)
                        }

                         
                        sqlDataSample.class  = classArray
                        
                        var vTotArray=[]
                        for(let z=0; z<vTot.length; z++)
                        {
                          
                          vTotArray.push(vTot[z].views_total)
                        }


                      sqlDataSample.views_total  = vTotArray
                      ...
                      if(typeof views_not_zero !='undefined'  && views_not_zero != null && views_not_zero != ''){
                        ...
                        for (var key in notZeroClass) {
                          var classSample ={}
                          classSample.class  = notZeroClass[key]
                          nZclass.push(classSample)
                        }
                        
                        for (var key in viewsNotZero){
                          var vNotSample = {}
                          vNotSample.views_total  = viewsNotZero[key]
                          nViews.push(vNotSample)
                        }
                       
                        for (var key in notZeroLocale){
                          var nZeroLocaleSample ={}
                          nZeroLocaleSample.nlocale = notZeroLocale[key]
                          nLocale.push(nZeroLocaleSample)
                        } 


                        var nLocaleArray=[]
                        for(let z=0;z<nLocale.length;z++){
                          nLocaleArray.push(nLocale[z].nlocale)
                        }

                        sqlDataSample.nlocale  = nLocaleArray
                      
                        var nZclassArray=[]
                        for(let z=0; z<nZclass.length; z++)
                        {
                          
                          
                          nZclassArray.push(nZclass[z].class)
                        }

                        sqlDataSample.not_zero_class  = nZclassArray
                        
                        var nViewsArray=[]
                        for(let z=0; z<nViews.length; z++)
                        {
                          
                          
                          nViewsArray.push(nViews[z].views_total)
                        }


                      sqlDataSample.views_not_zero_total  = nViewsArray
