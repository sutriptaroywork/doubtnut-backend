//Custom Version for mathjax node page  // if taking npm replace content of this file in main.js
...
MjPageJob.prototype.run = function() {
    ...
    while (script = scripts[index]) {
        ...
        typeset(conf, (result, options) => {  // async call
            ...
            if (result.errors) {
                ...
                _is_error = true;
                //console.log(this._outstandingHandlers)
                //window.close()
                //callback(false)
                return;
            }
    ...
    this.once('ready', () => {
        ...
        typeset(conf, (result) => {
            ...
            window.close();
            //ERROR HANDLING IF 1 Formula fails it returns false
            //reintilize is_error for next call
            if(_is_error){
                _is_error = false
                callback(false)
            }else{
