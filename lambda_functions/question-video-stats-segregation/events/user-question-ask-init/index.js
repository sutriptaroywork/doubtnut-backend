...
exports.handler = async (event, context) => {
	try {
		...
		if (dataToInsertQuestionInit.length > 0) {
			let r = await bulkInsertIntoQuestionInit(client_write,dataToInsertQuestionInit);
			console.log(r)
		}
...
function bulkInsertIntoQuestionInit (client, dataToInsert) {
	return new Promise(async function (resolve, reject) {
		var sql = 'INSERT INTO user_questions_init (uuid,' +
			' question_id,' +
			' student_id,' +
			' class, subject, book, chapter, question, doubt, locale, asked_at) VALUES ?';
