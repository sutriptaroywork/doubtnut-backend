...
var matchedQuestionsSourceSchema = Joi.object().keys({
...
var matchedQuestionsSchema = Joi.object().keys({
...
var feedbackSchema = Joi.object().keys({
  feedback_text: Joi.string(),
  is_show: Joi.number(),
  bg_color: Joi.string()
}).required();
const notificationArraySchema = Joi.array().items(notificationSchema).required();
const matchedQuestionsArraySchema = Joi.array().items(matchedQuestionsSchema).unique().required().options({ stripUnknown: true });
