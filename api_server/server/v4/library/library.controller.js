...
async function getAll(req, res, next) {
    try {
        ...
        const data = await libraryMysql.getPlaylistAllWithVersionCode(db.mysql.read, student_class, student_id, page_no, 10, version_code);
		 const responseData = {
	        meta: {
	          code: 200,
	          success: true,
...
async function getPlaylist(req, res, next) {
    try {
        ...
        // console.log(data1)
        if (data1.length !== 0) {
            const data2 = await libraryMysql.getParentData(db.mysql.read, student_class, data1[0].parent);
            if (page_no === '1') {
                // console.log(data2)
                data.header = data2;
            }
...
async function getResource(req, res, next) {
    try {
        ...
        if (id === 'SFY') {
            ...
            if (page_no === '1') {
                const promise = [];
                promise.push(libraryMysql.getParentData(db.mysql.read, student_class, null));
            ...
            if (data1.length !== 0) {
                ...
                if (data1[0].parent === '0') {
                    ...
                    data2 = await libraryMysql.getParentData(db.mysql.read, student_class, data1[0].parent);
                }
                if (page_no === '1') {
                    // console.log(data2)
                    data.header = data2;
                ...
                if (data3[0].is_last == 1) {
                    // let str=_.replace(data3[0].resource_path, /xxlanxx/g, language).replace(data3[0].resource_path, /xxclsxx/g, student_class).replace(data3[0].resource_path, /xxsidxx/g, student_id)
                    ...
                    for (let i = 0; i < data3.length; i++) {
                        /// /console.log(data3[i].ocr_text)
                        const str = data3[i].ocr_text.replace(/'/g, "\\'");
                        data3[i].ocr_text = str;
                    }
                }
                if (data3.length === 0 && page_no === '1' && (data1[0].description === 'HISTORY' || data1[0].description === 'DPP')) {
                    data.meta_info = [{
                        icon: `${config.staticCDN}images/empty_playlist.webp`, title: 'NO VIDEOS', description: data1[0].empty_text, Button: 'WATCH TRENDING VIDEOS', id: '100780', playlist_name: 'TRENDING ON DOUBTNUT',
                    }];
                } else if (data3.length === 0 && page_no === '1' && data1[0].description !== 'HISTORY' && data1[0].description !== 'TRENDING') {
