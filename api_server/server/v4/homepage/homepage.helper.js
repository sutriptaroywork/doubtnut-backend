/**
 * @Author: xesloohc
 * @Date:   2019-08-13T22:01:33+05:30
 * @Email:  god@xesloohc.com
 * @Last modified by:   xesloohc
 * @Last modified time: 2019-08-20T16:37:49+05:30
 */
            ...
            if(caraouselOrder[i].type == 'SFY'){
              ...
              if(output[i]['hits']['hits'].length > 0){
                ...
                for(let k=0;k<output[i]['hits']['hits'].length && k < 6;k++){
                  if(output[i]['hits']['hits'][k]['_id'] !== data['question_id']) {
                    ...
                    let i_url = `${config.staticCDN}q-thumbnail/` + output[i]['hits']['hits'][k]['_id'] + ".png"
                    let o = {
                      "id": output[i]['hits']['hits'][k]['_id'],
                      "type": "video",
                      "image_url": i_url,
                      "subject":output[i]['hits']['hits'][k]['_source']['subject'],
                      "is_locked":is_locked,
...
function generatePromises(caraouselOrder,data,db,elasticSearchInstance){
  ...
  for (let i = 0; i < caraouselOrder.length; i++) {
    ...
    if (caraouselOrder[i].type === 'DPP') {
      ...
      promise.push(elasticSearchInstance.findByOcrUsingIndexNew(data['ocr'],"doubtnut_new_physics_chemistry_maths_v2"))
    }
  }
  return promise
}
