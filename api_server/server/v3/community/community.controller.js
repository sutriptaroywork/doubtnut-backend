...
async function getCommunityMeta(req, res, next) {
  try {
    ...
    if(_.isEmpty(class1)){
      class1  = "'6','7','8','9','10','11','12'"
    }
      promises.push(Community.getCommunityChaptersWithLanguage(class1,language, db.mysql.read));
      promises.push(Community.getCommunitySubtopicsWithLanguage(class1,language, db.mysql.read));
      let resolvedPromises = await Promise.all(promises)
        data.chapter = resolvedPromises[0]
        data.subtopic = resolvedPromises[1]
      let responseData = {
        "meta": {
          "code": 200,
          "success": true,
...
function getCommunitySingleQuestion(req, res, next) {
  ...
  if (typeof question_id !== 'undefined' && !_.isNull(question_id)) {
    Question.getCommunityQuestionByQid(question_id, student_id, db.mysql.read).then(result => {
      //console.log(result)
