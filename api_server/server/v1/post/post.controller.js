 ...
 * @Last modified time: 2019-07-09T17:53:29+05:30
 */
...
async function add(req, res, next) {
    ...
    let resourceVideo_S3_Url = "";
    let contain_text = false;
    let contain_audio = false;
    let contain_video = false;
    ...
    let class_group = req.user.student_class;
    ...
    let checkbanned = await BannedUser.getBannedUserBystudentIdAndModule(db.mysql.read,student_id)
    ...
    if (resourceImage) {
      let resourceImageData = fs.readFileSync(resourceImage.path)
    ...
    if (resourceAudio) {
       let resourceAudioData = fs.readFileSync(resourceAudio.path)
