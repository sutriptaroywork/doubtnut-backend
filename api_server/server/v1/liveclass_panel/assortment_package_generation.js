...
function getAllFreeCourse(database) {
    ...
    const sql = 'select distinct assortment_id from course_details where is_free=0 and assortment_type  not in (\'chapter\', \'resource_video\', \'resource_pdf\', \'resource_test\',\'resource_quiz\')';
    return database.query(sql);
}
...
function getVideoResources(database) {
    // let sql = `SELECT * FROM (select * from course_details where assortment_id >=1541 and assortment_type = 'resource_video') as a left join (select assortment_id, schedule_type,course_resource_id from course_resource_mapping) as b on a.assortment_id=b.assortment_id left join (select * from course_resources where resource_type=4) as c on b.course_resource_id=c.id where b.assortment_id is not null and c.id is not null`;
    // let sql = `SELECT * FROM (select * from course_details where assortment_id >=1541 and assortment_type = 'resource_video') as a left join (select assortment_id, schedule_type,course_resource_id from course_resource_mapping) as b on a.assortment_id=b.assortment_id where b.assortment_id is not null`;
    const sql = 'SELECT distinct a.assortment_id, a.category, a.display_name from (SELECT *  FROM course_details WHERE assortment_type = \'resource_video\' and is_free = 0) as a left join package as b on a.assortment_id = b.assortment_id where b.id is null order by a.assortment_id asc';
    console.log(sql);
    return database.query(sql);
}
...
async function main(db) {
    try {
        const chapterResources = await getVideoResources(db.mysql.read);
        console.log(chapterResources);
        for (let i = 0; i < chapterResources.length; i++) {
            try {
                const assortmentType = await getAssortmentType(db.mysql.read, chapterResources[i].assortment_id);
                const check = await checkAssortmentInPackages(db.mysql.read, chapterResources[i].assortment_id);
        ...
        const pdfResources = await getPDFResources(db.mysql.read);
        console.log(pdfResources);

        for (let i = 0; i < pdfResources.length; i++) {
            const assortmentType = await getAssortmentType(db.mysql.read, pdfResources[i].assortment_id);
            const check = await checkAssortmentInPackages(db.mysql.read, pdfResources[i].assortment_id);
            if (assortmentType && assortmentType.length > 0 && !check.length) {
                if (assortmentType[0].schedule_type == 'recorded') {
                    const courseResource = await getCourseResource(db.mysql.read, assortmentType[0].course_resource_id);
        ...
        const testResources = await getTestResources(db.mysql.read);
        for (let i = 0; i < testResources.length; i++) {
            const assortmentType = await getAssortmentType(db.mysql.read, testResources[i].assortment_id);
            const check = await checkAssortmentInPackages(db.mysql.read, testResources[i].assortment_id);
            if (assortmentType && assortmentType.length > 0 && !check.length) {
                if (assortmentType[0].schedule_type == 'recorded') {
                    const courseResource = await getCourseResource(db.mysql.read, assortmentType[0].course_resource_id);
        ...
        const allFreeCourse = await getAllFreeCourse(db.mysql.read);
        for (let i = 0; i < allFreeCourse.length; i++) {
            const courseMapping = await getCourseMapping(db.mysql.read, allFreeCourse[i].assortment_id);
            if (courseMapping && courseMapping.length > 0) {
                for (let j = 0; j < courseMapping.length; j++) {
                    const type = await getType(db.mysql.read, courseMapping[j].course_resource_id);
                    const check = await checkAssortmentInPackages(db.mysql.read, courseMapping[j].course_resource_id);
                    if (type && type.length && !check.length) {
                        if (type[0].assortment_type == 'class') {
                            ...
                            const obj = {
                                ...
                                min_limit: 59,
                                duration_in_days: 365,
                                batch_id: courseMapping[j].batch_id,
