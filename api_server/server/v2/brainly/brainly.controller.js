...
async function search(req,res,next){
	try{
		...
		if(brainlyRes[0].length > 0)
		{
			...
			new_res.forEach(async value => {
				let likesViewRes = await QuestionContainer.getTotalViewsWeb(value._id, db)
				value.tot_likes = Math.floor(likesViewRes[1][0].total_likes)
				value.tot_count = Math.floor(likesViewRes[0][0].total_count)
			})
...
async function searchWithUrl(req,res,next){
	try{
		db = req.app.get('db')
		config = req.app.get('config')
		...
		let promise = []
		promise.push(brainlyContainer.getBrainlyQuestionWithUrl(url, db))
		...
		if(brainlyRes[0].length > 0)
		{
			...
			let data = { "question_details": qData }
...
module.exports = {search,searchWithUrl,feedback}
