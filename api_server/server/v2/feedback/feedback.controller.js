...
async function addVideoFeedback(req, res, next) {
  ...
  try {
    ...
    let answer_video = req.body.answer_video;
    ...
    video_feedback.view_time = view_time;
...
async function putMatchFailureFeedback(req, _res, next) {
    try {
        ...
        const elasticClient = req.app.get('elasticSearchTestInstance');
        let bookName = data.feedback;
        let studentClass = req.user.student_class;
        if (studentClass === 13 || studentClass === '13') {
            studentClass = 12;
        }
        if (bookName == 'mathematics' || bookName == 'mathamatics' || bookName == 'mathemateecs' || bookName == 'mathemetics' || bookName == 'mathematic' || bookName == 'methematics') {
            bookName = 'maths';
        } else if (bookName == 'pdf') {
            bookName = 'books';
        } else if (bookName == 'ganit' || bookName == 'ganit class 6' || bookName == 'ganit class 7' || bookName == 'ganit class 8' || bookName == 'ganit class 9' || bookName == 'ganit class 10' || bookName == 'ganit class 11' || bookName == 'ganit class 12') {
            bookName = 'maths';
        }
        const results = await elasticClient.getBooksSuggession(bookName, studentClass);
        if (results !== undefined) {
            const resultsDiff = Utility.stringDiffImplementWithKeyForBookFuzzy(results.hits.hits, bookName, fuzz, 'book_name', config.elastic.BOOK_FUZZY_INDEX);
            const res = [];
            for (let i = 0; i < resultsDiff.length; i++) {
                if (resultsDiff[i].partial_score > 50) {
                    res.push(resultsDiff[i]);
                }
            }
            const resultNew = {};
            resultNew.title = staticData.fuzzy_title;
            // resultNew.books = res.slice(0, 3);
            // resultNew.books = resultsDiff;
            resultNew.books = res;
            data = resultNew;
            next({ data });
        } else {
            data = {
                title: staticData.fuzzy_title,
                books: [],
            };
            next({ data });
        }
