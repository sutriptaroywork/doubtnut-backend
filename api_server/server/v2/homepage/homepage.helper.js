  ...
  // return new Promise(async function (resolve, reject) {
    ...
    try {
        ...
        for(let i=0;i<caraouselOrder.length;i++){
          if(caraouselOrder[i].type=='WHATSAPP_ASK' || caraouselOrder[i].data_type === 'topic' || caraouselOrder[i].data_type === 'ncert' || caraouselOrder[i].data_type === 'library_video')
          {
              ...
              if(output[i]['hits']['hits'].length > 0){
                ...
                for(let k=0;k<output[i]['hits']['hits'].length && k < 6;k++){
                  if(output[i]['hits']['hits'][k]['_source']['question_id'] !== data['question_id']) {
                    ...
                    let i_url = `${config.staticCDN}q-thumbnail/` + output[i]['hits']['hits'][k]['_source']['question_id'] + ".png"
                    let o = {
...
function generatePromises(caraouselOrder,data,db,elasticSearchInstance){
  ...
  for (let i = 0; i < caraouselOrder.length; i++) {
    ...
    if (caraouselOrder[i].type === 'DPP') {
      ...
      let gradient = _.sample(data['color'])
      promise.push(PlaylistContainer.getNCERTDataNewLibrary(caraouselOrder[i].type, gradient, caraouselOrder[i].data_type, data['description'], "", data['capsule'], data['student_class'], caraouselOrder[i].data_limit, data['language'],caraouselOrder[i].mapped_playlist_id, db))
    }
