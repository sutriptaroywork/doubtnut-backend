...
async function getFeed(req, res, next) {
  ...
  try {
    ...
    if(page_no==="1"){
      promises.push(MysqlFeed.getAppBannersNew(student_class, version_code,db.mysql.read))
    }
      ...
      let pinnedPost = await FeedContainer.getPinnedPost(student_class, filter, db)
        ...
        if (pinnedPost.length > 0) {
          ...
          if(pinnedPost[0]['post_type'] === "viral_videos") {
            ...
            let answer = await AnswerContainer.getByQuestionId(pinnedPost[0]['question_id'], db)
            sample.video_obj = {
                autoplay: true,
                video_url:'https://d3cvwyf9ksu0h5.cloudfront.net/'+answer[0]['answer_video'],
                question_id: pinnedPost[0]['question_id'],
