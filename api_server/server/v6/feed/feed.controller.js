...
async function getFeed(req, res, next) {
  ...
  try {
    ...
    //UNANSWERED
    let unanswered_qoutes = await Feed.getCustomQoutes(db.mysql.read,'UNANSWERED')
    if (filter == null) {
      if ((result[4].length !== 0)) {
        ...
        for (let i = 0; i < values.length; i++) {
          ...
          // value =  await Utility.getImage(value,FeedContainer,db)
          value =  Utility.changeUsernameNAvatar(student_class,value)
          value['text'] = _.shuffle(unanswered_qoutes)[0]['quote']
          value['id'] = value['id'].toString()
          data.push(value);
        }
      }
    ...
    let filtersToSend = Utility.generateFilters2(req)
    let responseData = {
      "meta": {
        "code": 200,
        "success": true,
...
async function getEntityDetails(req, res, next) {
  ...
  try {
    ...
    if(split_id.length > 0 && split_id[1] === "pinned"){
      // res.send("pinned")
      id = split_id[0]
      response = await MysqlFeed.getPinnedPostById(id,db.mysql.read)
      // let sample = {}
      // sample.id = response[0]['id']+"_pinned"
      // sample.type = response[0]['post_type']
      // sample.text = response[0]['title']
      // sample.student_id = response[0]['student_id']
      // sample.created_at = response[0]['created_at']
      // sample.image_url = response[0]['image_url']
      // sample.student_username = response[0]['student_username']
      // sample.profile_image = response[0]['profile_image']
      // if(response[0]['question_id'] !== null){
      //   sample.question_id = response[0]['question_id']
      // }
      let sample = {}
      sample.id = response[0]['id']+"_pinned"
      sample.type = response[0]['post_type']
      if(response[0]['post_type'] === "viral_videos") {
        sample.question_id = response[0]['question_id']
        let answer = await AnswerContainer.getByQuestionId(response[0]['question_id'], db)
        sample.video_obj = {
            autoplay: true,
            video_url:'https://d3cvwyf9ksu0h5.cloudfront.net/'+answer[0]['answer_video'],
            question_id: response[0]['question_id'],
      ...
      sample.created_at = response[0]['created_at']
      sample.image_url = response[0]['image_url']
      if(response[0]['audio_url'] !== null){
        sample.audio = response[0]['audio_url']
      }
      sample.profile_image = response[0]['profile_image']

      if(response[0]['post_type'] === "ugc"){
        sample.post_type = "JustAThought"
        sample.profile_image = `${config.staticCDN}images/logo.png`

      }
      if(response[0]['post_type'] === "youtube"){
        sample.youtube_id = response[0]['youtube_id']
