...
async function viewAnswerByQuestionId(req, res, next) {
    ...
    try {
        ...
        const Pre_Ad = await AnswerContainer.getPreAds(db);
        const Post_Ad = await AnswerContainer.getPostAds(db);

        //  //console.log('tot like >>>><<<<');
        // //console.log(Tot_Like);
        ...
        if (typeof page !== 'undefined' && typeof req.body.id !== 'undefined') {
            if (page === 'CC' || page === 'SC' || page === 'BROWSE_MC' || page === 'SEARCH_MC' || page === 'SEARCH_SC') { // CC = course chapter ; SC = subtopic chapter; BROWSE_MC - when user clicks on mc tag
                ...
                // console.log(resolvedPromisesData)
                const { question_id } = resolvedPromisesData[0];
                const Tot_Like = await AnswerContainer.getTotLikes(question_id, db);
                const answer_video_name = resolvedPromisesData[0].answer_video.split('.')[0];
                // let hls_url = "https://d1zcq8u9izvjk5.cloudfront.net/HLS/"+answer_video_name+"/"+answer_video_name+"-master-playlist.m3u8";
                ...
                if (resolvedPromisesData.length > 0) {
                    ...
                    if (Pre_Ad.length > 0) {
                        data.pre_add = Pre_Ad[0].ad_video;
                    }
                    if (Post_Ad.length > 0) {
                        data.post_add = Post_Ad[0].ad_video;
                    }
                    ...
                    const descrip = Utility.metaDataFunction(data);
                    // console.log(descrip);
                    if (descrip.length > 0) {
                        data.title = descrip[0];
                        data.description = descrip[1];
                        data.weburl = descrip[2];
                    }
                    data.total_likes = Tot_Like[0].total_likes;

                    // let weburl = Utility.webUrlFunction(data);
                    //   //console.log(weburl);
                    //   if(weburl.length>0){
                    //     data['weburl'] = weburl
                    //    }
                ...
                const promises = []; let questionWithAnswer;
                let wha_id = 0;
                if (typeof question_id !== 'undefined') {
                    ...
                    if (resolvedPromisesData.length > 0) {
                        ...
                        const Tot_Like = await AnswerContainer.getTotLikes(question_id, db);
                        // Saayon code _________________________________________________________________________________________________

                        const answer_video_name = resolvedPromisesData[0].answer_video.split('.')[0];
                        const hls_url = `https://d1zcq8u9izvjk5.cloudfront.net/HLS/${answer_video_name}/${answer_video_name}-master-playlist.m3u8`;
                        ...
                        if (parent_id != 0) {
                            ...
                            const findQuery = {
                                entity_id: parent_id, entity_type: 'matched', question_id: questionWithAnswer.question_id, is_deleted: false,
                            };
                            /*
    ...
    try {
        ...
        if (typeof page !== 'undefined') {
            if (page === 'SC' || page === 'CC' || page === 'BROWSE_MC') {
                ...
                let resolvedPromises; let doubt;
                // promises = []
...
async function onBoarding(req, res, next) {
    try {
        ...
        if (typeof page !== 'undefined' && typeof req.body.question_id !== 'undefined') {
            if (page === 'DOWNLOAD' || page === 'ONBOARDING' || page === 'STRUCTURED_ONBOARDING' || page === 'STRUCTURED' || page === 'COMMUNITY' || page === 'COURSE_DETAIL' || page === 'COURSE_FAQ' || page === 'NCERT' || page === 'REWARDS' || page === 'BOOK_LIST' || page.toLowerCase().includes('_auto') || page === 'REFERRAL_V2' || page === 'NCERT_NEW_FLOW') { // CC = course chapter ; SC = subtopic chapter; BROWSE_MC - when user clicks on mc tag
                // if(page )
                const { question_id } = req.body;
                if (typeof question_id !== 'undefined') {
                    if (typeof view_id !== 'undefined') {
                        await Answer.updateViewId(view_id, video_time, engage_time, db.mysql.write);
                        data = {
                            view_id,
                        };
                        const responseData = {
...
async function pdfDownload(req, res, next) {
    try {
        ...
        const params = { package: (req.body.package === undefined || req.body.package === '') ? null : req.body.package, level1: (req.body.level1 === undefined || req.body.level1 === '') ? null : req.body.level1, level2: (req.body.level2 === undefined || req.body.level2 === '') ? null : req.body.level2 };
        ...
        if (params.package == '' || _.isNull(params.package) || params.package === undefined) {
            ...
            if ((params.package == 'NCERT') && (params.level1 == '' || _.isNull(params.level1) || params.level1 === undefined)) {
                ...
                const responseData = {
                    ...
                    data: { filterType: 'level1', cdn_url: `${config.cdn_url}pdf_download/`, 'data-list': datas },
                ...
                const responseData = {
                    ...
                    data: { filterType: 'level2', cdn_url: `${config.cdn_url}pdf_download/`, 'data-list': datas },
...
async function videoPageDataWeb(req, res, next) {
    try {
        db = req.app.get('db');
        config = req.app.get('config');
        const question_id = req.body.id;
        ...
        if (typeof question_id !== 'undefined') {
            ...
            if (resolvedPromisesData.length > 0) {
                ...
                let ocr;
                let elasticSearchResult;
                if ((data.ocr_text.indexOf('<math')) == -1) {
                    ocr = data.ocr_text;
                } else {
                    ocr = data.question;
                }
                ...
                elasticSearchResult = quesres[1];
                elasticSearchResult = Utility.changeElasticSearchResult(elasticSearchResult.hits.hits);
                elasticSearchResult = Utility.addThumbnail(elasticSearchResult, config);
                data.matched_questions = elasticSearchResult;
                if (quesres[0][0]) {
                    data.question_meta = quesres[0][0];
                }
                const descrip = Utility.metaDataFunction(data);
                if (descrip.length > 0) {
                    data.title = descrip[0];
...
module.exports = {
    viewAnswerByQuestionId, updateAnswerView, viewSimilarQuestions, viewSimilarQuestionsWeb, onBoarding, pdfDownload, pdfDownloadWeb, videoPageDataWeb,
};
