...
const responseSchema = require('../../../responseModels/question/v6/question');
...
async function getMatchesByOcr(req, res, next) {
    try {
        db = req.app.get('db');
        ...
        const promise = [];
        promise.push(elasticSearchInstance.findByOcr(ocr_text));
        promise.push(AppConfiguration.isStringDiffActive(student_class, db.mysql.read));
        const result = await Promise.all(promise);
        const stringIsDiff = result[1][0].key_value;
        let matches_array;
        if (stringIsDiff) {
            matches_array = Utility.stringDiffImplement(result[0].hits.hits, ocr_text, fuzz);
        }
        const responseData = {
            meta: {
                code: 200,
...
async function ask(req, res, next) {
    ...
    try {
        ...
        if (qid) {
            ...
            master_iteration_mongo.ocr_type = ocr_data.ocr_type;
            ...
            Promise.all(promises).then((values) => {
                if (values[1]) {
                    ...
                    master_iteration_mongo.request_version = 'v6';
                            ...
                            for (let i = 0; i < values.length; i++) {
                                if (i == (values.length - 1)) {
                                    ...
                                    if (values[i].length > 0) {
                                        ...
                                        const result1 = await Promise.all(promisestemp);
                                        promisestemp = [];
                                        ...
                                        const result2 = await Promise.all(promisestemp);
                                        ...
                                        matches_array[i]._source.share_message = 'Waah! क्या बढ़िया तरीके से इस question ko Doubtnut App ने समझाया hai :D Khud dekho...maan jaaoge';
