...
```
# VS Code has inbuilt debugging capabilities for Javscript, Typescript 
1. Click on Run
2. Create the launch.json file, if not already created
    i.  Provide the starter file of project path in "program" attribute.
    ii. Refer the following launch.json file
3. Click on start debugging button which is on top-left, it will start the application in debugging mode
4. Set breakpoints where you want to debug
```
```javascript
{
...
## Mutli CDN Experiment [Doc](https://docs.google.com/document/d/1jps3wdwUTOX1luvw_AIgCu4L7CpIjoywrHqNSjSFEKw/edit?usp=sharing)
```
# Go through the doc to find the Mutli CDN Experiment
```
